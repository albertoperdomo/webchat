...
XMPPClient = function() {
    ...
    self.onRoster = function(ntype, buddy, subscription, ask) {}
...
//        console.log("connection opened");
    ...
    var nodeReceived = function(node) {
        ...
        if (node.nodeName == "message") {
            ...
            var c = node.childNodes;
            if (c[0].nodeName == "query" && c[0].getAttribute("xmlns") == "jabber:iq:roster") {
//               console.log("roster:");
//               console.log(node);
               var cc = c[0].childNodes;
               for(var i = 0; i < cc.length; i++) {
                   var ntype = node.getAttribute("type");
                   var buddy = cc[i].getAttribute("jid");
                   var subscription = cc[i].getAttribute("subscription");
                   var ask = cc[i].getAttribute("ask");
	           self.onRoster(ntype, buddy, subscription, ask);
               }
            }
	}
        else
